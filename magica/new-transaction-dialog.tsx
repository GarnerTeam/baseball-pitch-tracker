...
// ─── Checkbox group ───────────────────────────────────────────────────────────
function CheckGroup({
  label, options, selected, onChange, otherValue, onOtherChange, idPrefix, required, error,
}: {
  label: string; options: string[]; selected: string[]
  onChange: (v: string[]) => void; otherValue: string
  onOtherChange: (v: string) => void; idPrefix: string
        ...
        {options.map(opt => (
          <div key={opt}>
            <div className="flex items-start gap-2.5">
              ...
              />
              <label htmlFor={`${idPrefix}-${opt}`}
                className="text-sm text-gray-700 cursor-pointer select-none leading-snug">
                {opt}
              </label>
            </div>
            {opt === "Other" && hasOther && (
              <Input className="mt-2 ml-7 h-8 text-sm w-[calc(100%-1.75rem)]"
                placeholder="Please specify…" value={otherValue}
...
export function NewTransactionDialog() {
  ...
  const [noteToCoop,      setNoteToCoop]      = useState("")

  const [errors, setErrors] = useState<Record<string, string>>({})

  const reset = () => {
    setTeamMemberName(""); setFileType(""); setPresentationDate("")
    setC1Name(""); setC1Email(""); setC1Phone("")
    setAddress("")
    setC2Name(""); setC2Email(""); setC2Phone("")
    setPropChars([]); setPropCharsOther("")
    setPaperwork([]); setAgentRemarks("")
    setAgentEmail(""); setNoteToClient(""); setNoteToCoop("")
    setErrors({})
  }
  ...
  const handleLaunch = () => {
    ...
    const txn: Transaction = {
      ...
      noteToClient:   noteToClient.trim() || undefined,
      noteToCoop:     noteToCoop.trim()   || undefined,
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) reset(); setOpen(v) }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Complete the fields below to open a new file.</p>
          </DialogHeader>
          ...
          <div className="space-y-5 py-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input placeholder="Agent name" value={teamMemberName}
            ...
            <div className="w-1/2 pr-1.5">
              <Label className="text-xs text-gray-600 mb-1 block">
                Presentation Date <span className="text-gray-400 font-normal">(optional)</span>
              </Label>
              <Input type="date" value={presentationDate}
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input placeholder="John Smith" value={c1Name}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input type="email" placeholder="john@email.com" value={c1Email}
                  ...
                  <div>
                    ...
                    <Input placeholder="410-555-0100" value={c1Phone}
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  <Label className="text-xs text-gray-600 mb-1 block">Full Name</Label>
                  <Input placeholder="Jane Smith" value={c2Name} onChange={e => setC2Name(e.target.value)} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label className="text-xs text-gray-600 mb-1 block">Email</Label>
                    <Input type="email" placeholder="jane@email.com" value={c2Email} onChange={e => setC2Email(e.target.value)} />
                  </div>
                  <div>
                    <Label className="text-xs text-gray-600 mb-1 block">Phone</Label>
                    <Input placeholder="410-555-0101" value={c2Phone} onChange={e => setC2Phone(e.target.value)} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea placeholder="Any additional notes for the team…" value={agentRemarks}
                onChange={e => setAgentRemarks(e.target.value)} className="resize-none" rows={3} />
            </div>
            ...
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">
                Communications <span className="text-gray-300 font-normal normal-case tracking-normal text-xs">(optional)</span>
              </p>
              <div className="space-y-3">
                <div>
                  <Label className="text-xs text-gray-600 mb-1 block">Agent Email Address</Label>
                  <Input type="email" placeholder="agent@email.com" value={agentEmail}
                    onChange={e => setAgentEmail(e.target.value)} />
                </div>
                <div>
                  <Label className="text-xs text-gray-600 mb-1 block">Note to Client</Label>
                  <Textarea placeholder="Message that will be sent to your client…" value={noteToClient}
                    onChange={e => setNoteToClient(e.target.value)} className="resize-none" rows={2} />
                </div>
                <div>
                  <Label className="text-xs text-gray-600 mb-1 block">Note to Coop</Label>
                  <Textarea placeholder="Message for the cooperating agent…" value={noteToCoop}
                    onChange={e => setNoteToCoop(e.target.value)} className="resize-none" rows={2} />
                </div>
              </div>
            </div>

            {/* ── Subject Property + Characteristics (Seller/Landlord only) ── */}
            {(fileType === "Seller File" || fileType === "Landlord File") && (
              <>
                <Separator />
                ...
                <div>
                  ...
                  <Input placeholder="1234 Maple Drive, Columbia, MD 21044" value={address}
          ...
          <DialogFooter className="gap-2 pt-2">
            ...
            <Button onClick={handleLaunch} className="bg-[#012169] hover:bg-[#418FDE] text-white gap-2">
              <Rocket className="h-4 w-4" />
              Launch File
            </Button>
          </DialogFooter>
