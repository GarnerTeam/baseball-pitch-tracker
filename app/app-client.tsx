    ...
    <div className="fixed inset-0 h-dvh bg-slate-950 text-slate-100 flex flex-col">
      ...
      <div className="flex-1 min-h-0">
        ...
        {state.activeTab === 'lineup' && (
          <LineupPanel state={state} onNextBatter={actions.nextBatter} onPrevBatter={actions.prevBatter} onEndAtBat={actions.endAtBat} onChangePitcher={actions.changePitcher} onAddBatter={actions.addBatter} onRemoveBatter={actions.removeBatter} onSetBatterAt={actions.setBatterAt} onUndoLastEnd={actions.undoLastEnd} onSetWebhookUrl={actions.setSheetsUrl} onReorderBatter={actions.reorderBatter} onEditPitch={actions.editPitch} />
        )}
